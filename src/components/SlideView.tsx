...
export function SlideView({ slide }: { slide: Slide }) {
  return (
    <div className="w-full max-w-4xl text-center">
      {slide.subtitle && (
        <p className="text-sm font-medium uppercase tracking-[0.35em] text-indigo-400">
          <InlineText text={slide.subtitle} />
        </p>
      )}

      <h1 className="mt-6 text-5xl font-bold leading-tight tracking-tight text-white sm:text-7xl">
      ...
      {slide.items.length > 0 && (
        <ul className="mx-auto mt-12 max-w-3xl space-y-5 text-left">
          {slide.items.map((item, i) => (
            <li key={i} className="flex items-center gap-4 text-xl leading-snug text-slate-200 sm:text-2xl">
