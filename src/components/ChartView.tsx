...
export function ChartView({
    ...
    <>
      {series.map((entry, si) => (
        <g key={si}>
          <polyline
            points={entry.values
              .map((value, gi) => `${xFor(gi)},${yFor(value)}`)
              .join(" ")}
            fill="none"
  ...
  const renderPie = () => {
    ...
    labels.forEach((_, i) => {
      ...
      const a = polarPoint(PIE_CX, PIE_CY, PIE_RADIUS, start);
      const b = polarPoint(PIE_CX, PIE_CY, PIE_RADIUS, start + sweep);
      slices.push(
        <path
          key={i}
          d={`M ${a.x} ${a.y} A ${PIE_RADIUS} ${PIE_RADIUS} 0 ${largeArc} 1 ${b.x} ${b.y} L ${PIE_CX} ${PIE_CY} Z`}
    ...
    labels.forEach((_, i) => {
      ...
      if (shown.length < 7) {
        ...
        legend.push(
          <g
            key={i}
            transform={`translate(${PIE_LEGEND_X}, ${30 + shown.length * 22})`}
          >
            <circle cx={3} cy={0} r={4} fill={COLORS[i % COLORS.length]} />
            <text
              x={14}
              y={3}
              fontSize={11}
    ...
    <>
      ...
      {type === "pie" ? (
        <div className="text-muted">
          {pieValues[hover.index] ?? 0} ·{" "}
          {Math.round(((pieValues[hover.index] ?? 0) / pieTotal) * 100)}%
        </div>
      ) : (
        series.map((entry, si) => (
  ...
  return (
    ...
    >
      <svg
        viewBox={`0 0 ${type === "pie" ? PIE_W : W} ${type === "pie" ? PIE_H : H}`}
